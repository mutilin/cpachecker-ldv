...
int pdev;
struct ldv_thread *thread, *thread2;
...
int ieee80211_register_hw(void) {
   if(__VERIFIER_nondet_int()) {
      thread2 = ldv_thread_create(thread_ath9k, ((void *)0));
      return 0;
   }
   return -1;
}
...
void ieee80211_deregister_hw(void) {
   void *status;
   ldv_thread_join(thread_ath9k, thread2);
   return;
}
...
int module_init() {
   ...
   ldv_assert(pdev==1);
   if(__VERIFIER_nondet_int()) {
      thread = ldv_thread_create(thread_usb, ((void *)0));
      return 0;
   }
...
void module_exit() {
   ...
   ldv_thread_join(thread_usb, thread);
   pthread_mutex_destroy(&mutex);
